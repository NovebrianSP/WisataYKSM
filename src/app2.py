...
def rebuild_tfidf_model(df):
    ...
    # Create combined text field for TF-IDF with weighted features
    if "Description" in df.columns:
        # Berikan bobot lebih tinggi pada Category dan Price
        df["combined_text"] = (
            df["Place_Name"] + " " + 
            df["Category"].fillna("") * 3 + " " + 
            df["City"].astype(str) + " " + 
            df["Outdoor/Indoor"].fillna("") * 2 + " " + 
            df["Description"].fillna("") + " " + 
            df["Price"].astype(str) * 2
        )
    else:
        df["combined_text"] = (
            df["Place_Name"] + " " + 
...
def transform_similarity_score(sim_score):
    """Transform similarity score using sigmoid function to improve accuracy"""
    # Sigmoid function modified to output range 0-5
    return 5.0 / (1 + np.exp(-10 * (sim_score - 0.3)))

def get_user_preference_vector(username):
    """Get user preference vector for personalization"""
    db_path = os.path.join(BASE_DIR, "data/user_preferences.db")
    ...
    for pref in preferences:
        lokasi, _, _, kategori = pref
        
        # Add kategori to counter
        if kategori:
            for k in kategori.split(','):
                if k.strip():
                    kategori_counts[k.strip()] = kategori_counts.get(k.strip(), 0) + 1
        
        # Add lokasi to counter
        if lokasi:
            lokasi_counts[lokasi] = lokasi_counts.get(lokasi, 0) + 1
    
    return {
        'kategori_weights': kategori_counts,
        'lokasi_weights': lokasi_counts
    }

def record_user_feedback(username, destination_id, rating):
    ...
    # Save feedback
    cursor.execute('''
    INSERT INTO user_feedback (username, destination_id, rating)
    VALUES (?, ?, ?)
    ''', (username, destination_id, rating))
...
def get_content_based_recommendations_by_text(df, tfidf, tfidf_matrix, user_text, lokasi_terkini=None, cuaca_user=None, harga=None, top_n=50):
    ...
    # Add similarity scores to dataframe with improved transformation
    temp_df = df.copy()
    temp_df['raw_similarity'] = sim_scores
    temp_df['similarity_score'] = temp_df['raw_similarity'].apply(lambda x: min(x * 1.5, 1.0))
...
def get_hybrid_recommendations(df, tfidf, tfidf_matrix, username, lokasi_terkini, cuaca_user, user_text=None, harga=None):
    ...
    # Inisialisasi DataFrame kosong dengan kolom yang diperlukan
    all_recs = pd.DataFrame()
    
    # Get user's preference vector
    user_pref_vector = get_user_preference_vector(username)
    
    # Flag untuk menandai apakah ada rekomendasi kolaboratif
    has_collab_recs = False
    ...
    if similar_users:
        ...
        if not collaborative.empty and 'similarity_score' in collaborative.columns:
            collaborative['collab_score'] = collaborative['similarity_score']
            all_recs = pd.concat([all_recs, collaborative])
            has_collab_recs = True
    ...
    if not content_based.empty and 'similarity_score' in content_based.columns:
        content_based['content_score'] = content_based['similarity_score']
        all_recs = pd.concat([all_recs, content_based])
    
    # Compute hybrid scores with personalized weights
    if not all_recs.empty:
        # Remove duplicates, keeping highest scores
        all_recs = all_recs.drop_duplicates(subset=['Place_Name'], keep='first')
        
        # Inisialisasi kolom yang mungkin belum ada
        ...
        # Apply weighted ensemble with adjusted weights
        all_recs['similarity_score'] = (0.4 * all_recs['collab_score'] + 
                                      0.6 * all_recs['content_score'])
        
        # Sort by hybrid score
        all_recs = all_recs.sort_values('similarity_score', ascending=False)
                ...
                for j in range(n_cols):
                    ...
                    if idx < len(items):
                        ...
                        with cols[j]:
                            ...
                            if st.session_state.get("logged_in"):
                                feedback = st.slider(f"Beri rating untuk {row['Place_Name']}", 1, 5, 3)
                                if st.button(f"Kirim rating untuk {row['Place_Name']}", key=f"btn_{idx}"):
                                    record_user_feedback(st.session_state["username"], row['Place_Name'], feedback)
